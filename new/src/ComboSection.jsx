...
const ComboCard = ({ combo, onNavigate }) => (
  <div className="flex flex-col group cursor-pointer group/card" onClick={() => onNavigate && onNavigate(combo.id)}>
    ...
    <div
      className={`relative aspect-square rounded-[2px] overflow-hidden mb-6 bg-linear-to-br ${combo.color} border border-stone-100 shadow-sm group-hover/card:shadow-md transition-all duration-700 transform-gpu optimize-gpu`}
    >
      <img
        src={combo.image}
        alt={combo.name}
...
export default function ComboSection({ onNavigate }) {
  return (
