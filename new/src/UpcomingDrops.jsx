...
export default React.memo(function UpcomingDrops({ onNavigate }) {
  ...
  return (
    <section className="section pt-16 pb-8 px-4 bg-linear-to-br from-[#fdf6f9] via-[#f7f0ff] to-[#f0f7ff] relative overflow-hidden">
      {/* Background Decor */}
                ...
                >
                  ...
                  <div className="md:hidden bg-white/95 rounded-[32px] p-4 flex gap-5 items-center shadow-sm border border-white/50">
                    ...
                    <div className="flex-1 min-w-0">
                      <h3 className="text-base font-bold text-stone-900 leading-tight mb-1 uppercase tracking-tight">
                        {product.name}
                      </h3>
                      <p className="text-[12px] text-stone-400 font-medium mb-3 line-clamp-1">
                        {product.description}
                      </p>
                      <div className="flex items-center justify-between">
                        <span className="text-[20px] font-semibold text-[#151515]">
                          ₹{product.price.toLocaleString()}
                        </span>
                        <button className="bg-stone-900 text-white h-9 px-4 rounded-full text-[10px] font-black uppercase tracking-widest">
                          Remind
                        </button>
                      </div>
                    </div>
                  ...
                  <div className="hidden md:flex flex-col h-full bg-white/60 rounded-[40px] p-6 shadow-sm border border-white/60 hover:shadow-md hover:bg-white/90 transition-all duration-700 group relative overflow-hidden layer-isolate">
                    ...
                    <div className="px-1 flex-1 flex flex-col relative z-10">
                      <h3 className="text-lg font-bold text-stone-900 leading-[1.2] mb-2 uppercase tracking-tight group-hover:text-pink-500 transition-colors line-clamp-2">
                        {product.name}
                      </h3>
                      <p className="text-[13px] text-stone-500 font-light mb-6 line-clamp-2 leading-relaxed">
                        {product.description}
                      </p>
                      ...
                      <div className="mt-auto flex items-center justify-between gap-4">
                        <div className="flex flex-col">
                          <span className="text-[12px] font-bold text-stone-300 line-through leading-none mb-1">
                            ₹{product.originalPrice.toLocaleString()}
                          </span>
                          <span className="text-[20px] font-black text-[#151515] leading-none tracking-tighter">
                            ₹{product.price.toLocaleString()}
                          </span>
                        </div>
        ...
        {activeSlot.products.length === 0 && (
          <div className="text-center py-20 bg-white rounded-[32px] border-2 border-dashed border-pink-100 flex flex-col items-center gap-4">
            ...
            <p className="text-gray-400 font-black uppercase tracking-widest text-sm">
              No drops remaining for this slot
            </p>
          </div>
        )}
      </div>
