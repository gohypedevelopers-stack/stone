...
for i, line in enumerate(lines):
    if 'Primary Image{" "}' in line and 'Label' in line:
        start_idx = i - 1
    if 'Pricing Matrix' in line and 'h2' in line:
        end_idx = i - 11

if start_idx == -1 or end_idx == -1:
    # Use current line numbers from view_file (after previous restoration)
    start_idx = 6245 
    end_idx = 6405

print(f"Replacing from {start_idx} to {end_idx}")

new_section = r'''                                <div className="space-y-3">
                                  <Label
...
final_content = "".join(lines[:start_idx]) + new_section + "".join(lines[end_idx:])

with open(filepath, 'w', encoding='utf-8') as f:
    f.write(final_content)

print("Restored Product Media section successfully with fixed 'idx' logic")
