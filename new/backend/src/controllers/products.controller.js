...
export const listProducts = async (req, res) => {
  try {
    ...
    if (brand) {
      ...
      products = products.filter((product) => product.brand?.toLowerCase() === normalizedBrand);
    }
...
export const createProduct = async (req, res) => {
  try {
    ...
    if (!vendorId || !name || price === undefined) {
      return sendError(res, "Vendor, product name, and price are required", 400);
    }
    ...
    const product = await prisma.product.create({
      data: {
        ...
        discounted: discountPrice ? Number(discountPrice) < Number(price) : false,
